...
public:
  ...
  int* FindAllPosition(T value);//Поиск всех вхождения указанного значения
  size_t FindAllPositionNumber(T value);//Количество вхождений указанного значения
  void BubbleSort();
  void InsertionSort();
...
template<class T>
int* Vector<T>::FindAllPosition(T value)
{
  int* temp;
  int entry = 0;
  int ENTRYT = 0;
...
template<class T>
size_t Vector<T>::FindAllPositionNumber(T value)//количество вхождений указанного значения
{
  size_t count = 0;
  for (size_t i = 0; i < size; i++)
  {
